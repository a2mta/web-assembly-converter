...
#include <stdlib.h> // Add this line for abs, free, and calloc
#include <string.h> // Add this line for strcat and memset
...
lame_init_params(lame_global_flags * gfp)
{
    ...
    if (cfg->vbr == vbr_off && gfp->brate == 0) {
        ...
        if (fabsf(gfp->compression_ratio) == 0)
...
save_gain_values(lame_internal_flags * gfc)
{
    ...
    if (cfg->findReplayGain) {
        ...
        if (fabsf(RadioGain) != GAIN_NOT_ENOUGH_SAMPLES) {
